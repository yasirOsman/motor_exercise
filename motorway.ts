...
export let addToCache = (key: string,val: any) => {
  if(!cache.has(key)){
    cache.set(key,val);
  }
}
...
export const getFromCache = async(numberPlate: string) => {
    const price = await cache.get(numberPlate);
    return price;
}


export const getPrice = async (numberPlate: string, skipCacheForRead: boolean = true) => {
